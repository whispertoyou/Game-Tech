...
GLuint CommonMeshes::m_RaptorTex = 0;
...
void CommonMeshes::InitializeMeshes()
{
	if (m_pPlane == NULL)
	{
		...
		//new Raptor
		m_RaptorTex = SOIL_load_OGL_texture(TEXTUREDIR"raptor.jpg", SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS | SOIL_FLAG_NTSC_SAFE_RGB | SOIL_FLAG_COMPRESS_TO_DXT);
		...
		//new Raptor
		m_pRaptor->SetTexture(m_RaptorTex);
	}
}
